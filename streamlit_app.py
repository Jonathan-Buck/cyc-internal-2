...
# GPA ANALYSIS
def GPA(data):
    return sns.histplot(data = data, x = "GPA", kde=True, legend=False, bins = 20)

df_nonzero = df.replace(0, np.nan)
df_ag_nonzero = df_ag.replace(0, np.nan)
...
gpa_graphs, axes = plt.subplots(1, 2, figsize=(20, 10))
gpa_graphs.suptitle('GPA Analysis')

sns.histplot(ax=axes[0], data = df_nonzero, x = "GPA", kde=True, legend=False, bins = 20)
axes[0].set_title('Fall 2022')

sns.histplot(ax=axes[1], data = df_ag_nonzero, x = "GPA", kde=True, legend=False, bins = 20)
